...
import { claimSumUpEventOperation, refreshSumUpEventOperation, releaseSumUpEventOperation, startSumUpEventOperationHeartbeat } from "@/lib/sumup-event-operation"
...
export async function createOrder(data: {
    ...
    let sumUpEventOperationHeartbeat: ReturnType<typeof startSumUpEventOperationHeartbeat> | undefined
    try {
        const sessionCheck = await ensurePosActionSession()
        if (!sessionCheck.success) return sessionCheck
        ...
        if (requiresPendingState) {
            ...
            sumUpEventOperationHeartbeat = startSumUpEventOperationHeartbeat(data.eventId, sumUpEventOperationToken)
        }
        ...
        if (sumUpEventOperationHeartbeat && !await sumUpEventOperationHeartbeat.ensureOwned()) {
            await releaseCashSessionPaymentClaim(paymentClaimSessionId, paymentClaimToken)
            paymentClaimToken = undefined
        ...
        if (requiresPendingState) {
            ...
            const initiationResult = await sumUpEventOperationHeartbeat?.ensureOwned() ? await Order.updateOne(
            ...
            const sumupResult = await startPreparedSumUpPayment({
                eventId: data.eventId,
                posDeviceId: data.posDeviceId,
                orderId: order._id.toString(),
                amount: payableAmount,
                stockAdjustments: pendingStockAdjustments,
                eventOperationToken: sumUpEventOperationToken
            })
            if (!sumupResult.success) {
                ...
                return { success: false, error: sumupResult.error }
            }
        }
        ...
        sumUpEventOperationHeartbeat?.stop()
...
async function startPreparedSumUpPayment(params: {
    eventId: string
    posDeviceId: string | undefined
    orderId: string
    amount: number
    stockAdjustments: StockAdjustment[]
    eventOperationToken?: string | null
}): Promise<
    | { success: true; checkoutId: string }
    | { success: false; error: string; paymentUncertain?: boolean }
> {
    const stockTransition = {
        eventId: params.eventId,
        orderId: params.orderId,
        adjustments: params.stockAdjustments
    }
    const reservation = await transitionSumUpOrderStock({
        ...stockTransition,
        token: `SUMUP_RESERVE:${params.orderId}`,
        target: "APPLIED"
    })
    if (!reservation.success) return reservation

    const result = await triggerSumUpPaymentWithLease(
        params.amount,
        params.eventId,
        params.posDeviceId,
        params.orderId,
        params.eventOperationToken
    )
    if (result.success && result.checkoutId) return { success: true, checkoutId: result.checkoutId }

    const error = result.error || "Errore durante l'inizializzazione del pagamento elettronico"
    if ("paymentUncertain" in result && result.paymentUncertain) {
        return { success: false, error, paymentUncertain: true }
    }
    const release = await transitionSumUpOrderStock({
        ...stockTransition,
        token: `SUMUP_RELEASE:${params.orderId}`,
        target: "REVERTED"
    })
    return { success: false, error, paymentUncertain: !release.success }
}
...
async function triggerSumUpPaymentWithLease(
    ...
    try {
        ...
        if (ownsEventOperation) eventOperationHeartbeat = startSumUpEventOperationHeartbeat(eventId, eventOperationToken)
        if (!await refreshSumUpEventOperation(eventId, eventOperationToken)) {
            return { success: false, error: "Operazione SumUp non più esclusiva: riprova" }
        }
        ...
        const result = await refreshSumUpEventOperation(eventId, eventOperationToken).catch(() => false) ? await createSumUpCheckout({
            amount,
            currency: "EUR",
            merchantCode,
            readerId,
        ...
        }) : { success: false as const, error: "Operazione SumUp non più esclusiva: riprova", uncertain: false }
...
export async function completePendingOrderPayment(data: {
    ...
    try {
        ...
        if (isSumUpPayment) {
            ...
            const initiationMarker = `initiating:${order._id.toString()}`
            const claimedOrder = await sumUpEventOperationHeartbeat?.ensureOwned() ? await Order.updateOne(

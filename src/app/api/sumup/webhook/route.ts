...
async function loadWebhookOrder(clientTransactionId: string) {
    return await Order.findOne({ sumupCheckoutId: clientTransactionId })
        .select("_id status totalAmount eventId cashSessionId posDeviceId stockEffectStatus stockAdjustments sumupCheckoutId sumupPaymentId sumupPrintCompletedAt sumupRecoveryCancelledAt sumupLateSuccessDetectedAt stornoMeta.refundStatus +sumupRefundCredentials")
        .lean() as WebhookOrder | null
}

async function handlePaidCallback(order: WebhookOrder, outcome: ReturnType<typeof getSumUpTransactionOutcome>) {
    if (!order.sumupPrintCompletedAt) {
        if (outcome !== "SUCCESS") {
            return NextResponse.json({ error: "Paid transaction is no longer successful" }, { status: 409 })
        }
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (order?.status === "PAID" && order.sumupPrintCompletedAt) {
            return NextResponse.json({ success: true, message: "Already paid" })
        }
        ...
        if (order.status === "PAID") {
            return await handlePaidCallback(order, outcome)
        }
        ...
        if (!claimedOrder) {
            ...
            if (currentOrder?.status === "PAID") {
                return await handlePaidCallback(currentOrder, outcome)
            }

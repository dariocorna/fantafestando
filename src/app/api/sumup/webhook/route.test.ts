...
describe("POST /api/sumup/webhook", () => {
    ...
    test.each(["PENDING", "PAID"] as const)("dispatches an accepted %s payment after another event becomes active", async (status) => {
        const inactiveEvent = { _id: "event-1", active: false }
        orderFindOneMock.mockReturnValue(selectLean({ ...pendingOrder, status }))
        claimSumUpEventOperationMock.mockImplementation(async (eventId, activeOnly = false) =>
            eventId === inactiveEvent._id && (!activeOnly || inactiveEvent.active) ? "event-operation-1" : null
        )
    ...
    test("returns 503 for an already-paid callback while a print recovery claim is active, then completes after reclaim", async () => {
        ...
        expect(getByClientMock).toHaveBeenCalledTimes(2)
    })

    test.each([
        { name: "sent", printResults: [true] },
        { name: "terminal duplicate", printResults: [false] },
        { name: "no saved intents", printResults: [] },
    ])("recovers an already-paid callback idempotently after rechecking SumUp ($name)", async ({ printResults }) => {
        orderFindOneMock.mockReturnValue(selectLean({ ...pendingOrder, status: "PAID" }))
        routeOrderToPrintersMock.mockResolvedValue(printResults)

        const response = await POST(webhookRequest())

        expect(response.status).toBe(200)
        await expect(response.json()).resolves.toEqual({ success: true, message: "Already paid" })
        expect(getByClientMock).toHaveBeenCalledOnce()
        expect(routeOrderToPrintersMock).toHaveBeenCalledOnce()
        expect(routeOrderToPrintersMock).toHaveBeenCalledWith(
            "order-1",
            "pos-1",
    ...
    test.each(["REFUNDED", "CHARGEBACK"] as const)("does not resume paid print recovery after SumUp reports %s", async (simple_status) => {
        orderFindOneMock.mockReturnValue(selectLean({ ...pendingOrder, status: "PAID" }))
        routeOrderToPrintersMock.mockResolvedValue(["RECOVERY_PENDING"])
        const firstResponse = await POST(webhookRequest())
        expect(firstResponse.status).toBe(503)

        getByClientMock.mockResolvedValue({ success: true, transaction: { ...successfulTransaction, simple_status } })
    ...
    test("does not print a concurrently paid order after verifying a negative outcome", async () => {
        orderFindOneMock
            .mockReturnValueOnce(selectLean(pendingOrder))
            .mockReturnValueOnce(selectLean({ ...pendingOrder, status: "PAID" }))
        orderFindOneAndUpdateMock.mockResolvedValue(null)
        getByClientMock.mockResolvedValue({
            success: true, transaction: { ...successfulTransaction, simple_status: "REFUNDED" }
        })
    ...
    test("acknowledges a paid order with completed prints without querying SumUp or dispatching again", async () => {
        orderFindOneMock.mockReturnValue(selectLean({
            ...pendingOrder, status: "PAID", sumupPrintCompletedAt: new Date()
        }))

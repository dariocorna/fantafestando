...
function mockSumUpPeripheral(config: Record<string, string> = {
    merchantCode: "MK10CL2A",
    readerId: "rdr_3MSAFM23CK82VSTT4BN6RWSQ65",
    ...
    mocks.peripheralFindOne.mockReturnValue({
        lean: vi.fn().mockResolvedValue({ _id: "peripheral-1", type: "SUMUP", config })
    });
}
...
function legacySumUpRefundQuery(posDeviceId: string | { $in: string[] }) {
    return {
        ...
        $or: [expect.objectContaining({ status: "PAID" }), expect.objectContaining({ status: "CANCELLED" })]
    };
}

function activeSumUpCheckoutOrPrintQuery(posDeviceId: string | { $in: string[] }) {
    return {
        eventId: "event-1",
        posDeviceId,
        $or: [
            expect.objectContaining({ status: "PENDING" }),
            expect.objectContaining({ status: "PAID", sumupPrintCompletedAt: { $exists: false } })
        ]
    };
}
...
describe("SumUp peripheral configuration", () => {
    ...
    test.each([
        [{ merchantId: " legacy-merchant " }, " legacy-merchant ", true, true],
        [{ merchantCode: " current ", merchantId: "old" }, "current", true, true],
        [{ merchantId: "legacy-merchant" }, "different", true, false],
        [{ merchantCode: "current", merchantId: "old" }, "old", true, false],
        [{ merchantId: "legacy-merchant" }, "different", false, true],
        [{}, "new-merchant", true, false]
    ] as Array<[Record<string, string>, string, boolean, boolean]>)(
        "preserves merchant identity for legacy refunds (%j -> %s, dependency %s)",
        async (storedMerchant, merchantCode, hasDependency, allowed) => {
            mockSumUpPeripheral({ ...storedMerchant, affiliateKey: "encrypted:legacy-api-key" });
            mocks.posDeviceDistinct.mockResolvedValue(["pos-1"]);
            mocks.orderExists.mockResolvedValueOnce(false).mockResolvedValue(hasDependency);

            const result = await updatePeripheralAction(sumUpForm({ merchantCode }));
            ...
            if (allowed) {
                ...
                expect(mocks.peripheralFindOneAndUpdate).toHaveBeenCalledWith(
                    ...
                    expect.objectContaining({ config: expect.objectContaining({
                        ...
                        apiKey: "encrypted:sup_sk_api", affiliateKey: "encrypted:affiliate-secret"
                    }) }),
                    { returnDocument: "after" }
                );
            } else {
                expect(result).toEqual({ error: expect.stringMatching(/pagamento SumUp non ancora rimborsato/i) });
                expect(mocks.encryptSecret).not.toHaveBeenCalled();
                expect(mocks.peripheralFindOneAndUpdate).not.toHaveBeenCalled();
            }

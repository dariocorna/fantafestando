...
import { noActivePaymentClaim } from "@/lib/cash-session-payment-claim";
...
async function findBlockingSumUpOrder(sessionId: string) {
...
export async function setCashSessionTestAction(sessionId: string, isTest: boolean) {
    ...
    const blockingOrder = isTest ? await findBlockingSumUpOrder(sessionId) : null;
    if (blockingOrder) {
        ...
        return {
            ...
            error: blockingOrder.status === "PENDING"
                ? "Completa o annulla i pagamenti SumUp in attesa prima di classificare la sessione come TEST"
                : "Storna e rimborsa i pagamenti SumUp prima di classificare la sessione come TEST"
        };
    }
...
export async function deleteCashSessionAction(sessionId: string, confirmation: string) {
    ...
    const blockingOrder = await findBlockingSumUpOrder(sessionId);
    if (blockingOrder) {
        await releaseDeleteClaim();
        return {
            success: false as const,
            error: blockingOrder.status === "PENDING"
                ? "Completa o annulla i pagamenti SumUp in attesa prima di eliminare la sessione"
                : "Storna e rimborsa i pagamenti SumUp prima di eliminare la sessione"
        };
    }

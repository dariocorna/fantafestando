...
async function resumeInterruptedOpposingTransition(params: {
    ...
    try {
        const result = await transitionSumUpOrderStock({ ...params, token, target })
        if (!result.success && await isSumUpOrderAtTarget(params)) return { success: true as const }
        return result
    } catch (error) {
        if (await isSumUpOrderAtTarget(params)) return { success: true as const }
        throw error
    }
}

...
describe("PrinterService.routeOrderToPrinters", () => {
    ...
    test("keeps cashier summary, fallback department copy and customer copy ordered on the same printer", async () => {
        ...
        expect(printComandaSpy).toHaveBeenNthCalledWith(
            2,
            expect.objectContaining({ printType: "KITCHEN_ORDER", ip: "192.168.178.203", port: 9100 }),
            1
        );
        expect(printComandaSpy).toHaveBeenNthCalledWith(
            3,
    ...
    test("serializes jobs across concurrent orders that target the same printer", async () => {
        ...
        expect(printComandaSpy.mock.calls[1]?.[0]).toMatchObject({ orderId: "order-1", printType: "KITCHEN_ORDER" });
        expect(printComandaSpy.mock.calls[2]?.[0]).toMatchObject({ orderId: "order-1", printType: "CUSTOMER_ORDER" });
        ...
        expect(firstResult).toEqual([true, true, true]);
        expect(secondResult).toEqual([true, true, true]);
        expect(printComandaSpy.mock.calls.slice(0, 3).every(([job]) => job.orderId === "order-1")).toBe(true);
        expect(printComandaSpy.mock.calls.slice(3).every(([job]) => job.orderId === "order-2")).toBe(true);
        expect(printComandaSpy).toHaveBeenNthCalledWith(
            6,
    ...
    test("keeps fallback department jobs separated for categories without a kitchen printer", async () => {
        mockOrder({
        ...
        expect(kitchenJobs.map((job) => getPrintedItemNames(job)).sort()).toEqual([["Panino"], ["Patatine"]]);
        expect(customerJobs.map((job) => getPrintedItemNames(job)).sort()).toEqual([["Panino"], ["Patatine"]]);
        expect(kitchenJobs.every((job) => job.ip === "192.168.178.203")).toBe(true);
        expect(customerJobs.every((job) => job.ip === "192.168.178.203")).toBe(true);
    });

    test("keeps dedicated kitchen prints and fallback department prints distinct", async () => {
        mockOrder({
        ...
        expect(kitchenJobs).toEqual(
            expect.arrayContaining([
                ...
                expect.objectContaining({
                    ip: "192.168.178.203",
                    items: [expect.objectContaining({ name: "Patatine" })]
                })
            ])
        );
    ...
    test("fails fast on remaining jobs that share a destination after the first failure", async () => {
        ...
        expect(printComandaSpy.mock.calls[2]?.[2]).toEqual(
            expect.objectContaining({ immediateFailureReason: "Skipped after previous destination failure" })
        );
    });
});

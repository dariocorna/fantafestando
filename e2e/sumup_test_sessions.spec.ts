...
function paymentOrderData(eventId: string, product: { _id: string, name: string }) {
    return {
        eventId,
        totalAmount: 5,
        discountApplied: 0,
...
test.describe("SumUp e sessioni TEST", () => {
    ...
    test("riclassifica il POS manuale ma blocca un pagamento SumUp certificato", async ({ page, isMobile }) => {
        ...
        try {
            ...
            const commonOrder = {
                ...paymentOrderData(eventId, product!),
                status: "PAID",
                paidAt: new Date(),
    ...
    test("espone recovery incerta e pagamenti tardivi anche nelle sessioni TEST", async ({ page, isMobile }) => {
        ...
        try {
            ...
            const commonOrder = paymentOrderData(eventId, product!)

...
test.describe("Gestione Hardware ed Elettronica", () => {
    ...
    test("configurazione completa: stampante -> pos -> categoria", async ({ page }) => {
        ...
        await expect(page.getByText(printerPort)).toBeVisible();
        ...
        await cashierDialog.getByLabel("Porta TCP").fill(String(19100 + Math.floor(Math.random() * 10)));
        ...
        const expectedLabel = `${printerName} (${printerIp}:${printerPort})`;
        await page.selectOption('select[id="printerId"]', { label: expectedLabel });
    ...
    test("modifica hardware esistente (Full CRUD)", async ({ page }) => {
        ...
        await createDialog.getByLabel("Porta TCP").fill("19100");

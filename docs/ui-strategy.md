...
- **Tastiera Mirata (Keyboard on-demand)**: Il nucleo della spesa è gestito senza tasto fisico (le card prodotto). La tastiera a schermo (o quella fisica del PC) apparirà o sarà usata **solo ed esclusivamente** quando si deve fare una ricerca esplicita (es. cercare il Codice Breve di un Ordine Cloud), o quando si compila testualmente un'anagrafica.
- **Supporto Ibrido Touch/Mouse**: Il gestionale cassa girerà indistintamente su Tablet o su PC. L'interfaccia deve prevedere l'utilizzo ibrido in modo eccellente:
  - Lato touch: Hitbox/pad generosi per i bottoni.
  - Lato mouse: Pieno supporto agli stati `:hover` CSS per il feedback visivo immediato di cliccabilità, cursori `pointer` corretti, e scrollbar chiaramente afferrabili col mouse, per evitare la sensazione di usare un'app mobile "azzoppata" su desktop.
- **Adattamento Desktop/Tablet (>= 13 pollici)**: L'interfaccia non necessita di bottoni spropositatamente "enormi", poiché lo spazio disponibile su classici monitor o tablet da >= 13 pollici consente una densità visiva maggiore ma comunque altamente leggibile. Le card saranno proporzionate per garantire l'input touch, ma abbastanza contenute da favorire la vista d'insieme su laptop.
